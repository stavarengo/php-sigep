...
//			exit;
    ...
    public function calcPrecoPrazo(\PhpSigep\Model\CalcPrecoPrazo $params)
    {
        ...
        $soapArgs = array(
            ...
            'nVlComprimento'      => ($comprimento < $comprimentoMinimo && $params->getAjustarDimensaoMinima(
                ) ? $comprimentoMinimo : $comprimento),
            'nVlAltura'           => ($altura < $alturaMinima && $params->getAjustarDimensaoMinima(
                ) ? $alturaMinima : $altura),
            'nVlLargura'          => ($largura < $larguraMinima && $params->getAjustarDimensaoMinima(
                ) ? $larguraMinima : $largura),
            'nVlDiametro'         => ($diametro < $diametroMinimo && $params->getAjustarDimensaoMinima(
                ) ? $diametroMinimo : $largura),
                ...
                if (is_object($r->CalcPrecoPrazoResult->Servicos) && $r->CalcPrecoPrazoResult->Servicos->cServico) {
                    ...
                    foreach ($servicos as $servico) {
                        $item = new \PhpSigep\Model\CalcPrecoPrazoResposta(array(
                                                                               ...
                                                                               'valorMaoPropria'       => (float)str_replace(
                                                                                       ',',
                                                                                       '.',
                                                                                       str_replace(
                                                                                           '.',
                                                                                           '',
                                                                                           $servico->ValorMaoPropria
                                                                                       )
                                                                                   ),
                                                                               'valorAvisoRecebimento' => (float)str_replace(
                                                                                       ',',
                                                                                       '.',
                                                                                       str_replace(
                                                                                           '.',
                                                                                           '',
                                                                                           $servico->ValorAvisoRecebimento
                                                                                       )
                                                                                   ),
                                                                               'valorValorDeclarado'   => (float)str_replace(
                                                                                       ',',
                                                                                       '.',
                                                                                       str_replace(
                                                                                           '.',
                                                                                           '',
                                                                                           $servico->ValorValorDeclarado
                                                                                       )
                                                                                   ),
